 ...
 * @author yohanna
 */
public class TejidoNuevo <T> {
  private ListaVertice<T> lista_vertices=new ListaVertice<T>();
	private ListaArco<T> lista_arcos=new ListaArco<T>();
	public int costo1;
	 TejidoNuevo(){	
	}
	...
	public void AgregarArco(int vi,int vj, int costo){
		Arco<T> obj_arco=new Arco<T>(vi,vj,costo);
	...
	public void MarcarVertice(TejidoNuevo<T> g,int subindice){
		lista_vertices.BuscarVertice(subindice, lista_vertices);
		lista_vertices.getVentana().getVertice().setMarca(true);
	}
	
	public void DesmarcarVertice(TejidoNuevo<T> g,int subindice){
		lista_vertices.BuscarVertice(subindice, lista_vertices);
		lista_vertices.getVentana().getVertice().setMarca(false);
	}
	
	public boolean MarcadoVertice(TejidoNuevo<T>g, int subindice){
		lista_vertices.BuscarVertice(subindice, lista_vertices);
		return lista_vertices.getVentana().getVertice().getMarca();
	}
	public void DesmarcarGrafo(TejidoNuevo<T> g){
	...
	public Lista<T> Camino(TejidoNuevo<T>g, int v1, int v2){
		int menor;
		Lista<T> suc=new Lista<T>();
		Lista<T> resp1=new Lista<T>();
		Lista<T> resp2=new Lista<T>();
	...
	public int SiguienteVertice(TejidoNuevo<T> g,int costo[],int num_elem){
	...
	public int[] Dijkstra(TejidoNuevo<T> g, int v){
	 ...
	 public int min(int a,int b){
         ...
         public void crearVertices(Celula celulaInicial){
            System.out.println("aqui");
            int numVecinos;
            numVecinos=celulaInicial.getNumLado();
            System.out.println("aqui");
